...
function vppn_match;
    ...
    begin
        vppn_match = 1'b1;
        for (bit_i = 0; bit_i < 19; bit_i = bit_i + 1) begin
            if ((bit_i + 13) >= (entry_ps + 1)) begin
                if (search_vppn[bit_i] != entry_vppn[bit_i]) begin
                    vppn_match = 1'b0;
                end
            end
        end
...
function odd_page;
    ...
    integer sel_bit;
    begin
        if (entry_ps <= 6'd12) begin
            odd_page = va_bit12;
        end
        else begin
            sel_bit  = entry_ps - 6'd13;
            odd_page = (sel_bit < 19) ? search_vppn[sel_bit] : 1'b0;
        end
    end
endfunction
