...
// Redirect arbitration followed by a register.  The pipeline position
// already establishes request age: branch_valid is the registered EX1
// resolution (now in EX2), while IBAR/exception/ERTN are generated by the
// older instruction at the EX3 commit point.  A commit redirect therefore
// always wins over a simultaneous branch redirect.  The three commit causes
// share one instruction and use the fixed architectural priority below.
//
// Avoid re-comparing the 32-bit sequence IDs here.  Besides being redundant,
// those comparisons sit after the EX3 memory/LLSC readiness logic and form a
// carry-chain path into the wide redirect register.
